...
for fasta_file in $fasta_files; do
    ...
    find . -maxdepth 1 -type f ! \( -name "*.fasta" -o -name "*.success" \
        ...
        -o -name "modeller_automodel.py" -o -name "split.sh" -o -name "pdb_95.pir" -o -name "pdb_95.bin" -o -name "run.sh" \) \
        -exec mv '{}' "${base_name}/" \;
...
echo "Moving all directories to ../output/modeller_output..."
mkdir -p ../output/modeller_output  # Ensure the target directory exists
find . -maxdepth 1 -type d -not -path "." -not -name "scwrl4" -exec mv {} ../output/modeller_output/ \;
